  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      targetList.forEach(v => {
        ...
        if (st.includes('TRANSIT') || st.includes('ACTIVE')) inTransitCount++;
      });

      // Get logged-in user details for "Generated By"
      let officerName = 'RAB Official Auditor';
      try {
        const stored = localStorage.getItem('user');
        if (stored) {
          const parsed = JSON.parse(stored);
          let rawName = parsed.name || parsed.fullName || parsed.username || 'RAB Official Auditor';
          officerName = rawName.replace(/Super Admin/gi, 'RAB Officer').replace(/\s*\(RAB\)/gi, '').trim();
        }
      } catch (e) {}
        ...
        <head>
          ...
          <style>
            * { box-sizing: border-box; }
            body { margin: 0; padding: 0; background: #fff; font-family: Arial, Helvetica, sans-serif; color: #1e293b; font-size: 10.5px; }
            .report-container { width: 794px; padding: 24px 32px; margin: 0 auto; background: #fff; }
            .page-flex { min-height: 1040px; display: flex; flex-direction: column; justify-content: space-between; }
            
            .header-layout { display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; }
            .logo-group { display: flex; align-items: center; gap: 12px; }
            .header-text h1 { font-size: 17px; font-weight: 800; color: #0f172a; margin: 0; letter-spacing: -0.2px; }
            .header-text p { font-size: 10px; color: #64748b; margin: 3px 0 0 0; }
            
            .divider-line { height: 1px; background: #e2e8f0; margin: 10px 0 14px 0; }
            
            .metrics-table { width: 100%; border-collapse: collapse; margin-bottom: 16px; }
            .metrics-table td { vertical-align: top; padding: 0 12px 0 0; }
            .metric-label { font-size: 9.5px; color: #94a3b8; font-weight: 600; margin-bottom: 2px; }
            .metric-value { font-size: 13.5px; font-weight: 800; color: #0f172a; }
            
            table.rbc-table { width: 100%; border-collapse: collapse; margin-top: 8px; font-size: 9.5px; border: 1px solid #00a3e0; page-break-inside: auto; }
            table.rbc-table tr { page-break-inside: avoid; page-break-after: auto; }
            table.rbc-table th { background-color: #00a3e0; color: #ffffff; font-weight: 700; font-size: 9px; padding: 8px 6px; border: 1px solid #00a3e0; text-align: left; vertical-align: middle; }
            table.rbc-table td { border: 1px solid #00a3e0; padding: 7px 6px; color: #0f172a; vertical-align: middle; background: #ffffff; }
            table.rbc-table tr:nth-child(even) td { background-color: #f0f9ff; }
            
            .badge { display: inline-block; padding: 2px 5px; border-radius: 3px; font-size: 8px; font-weight: 700; background: #e0f2fe; color: #0369a1; }
            .badge-active { background: #dcfce7; color: #15803d; }
            
            .footer-section { border-top: 1px solid #e2e8f0; padding-top: 8px; margin-top: 16px; font-size: 8.5px; color: #64748b; }
          </style>
        </head>
        <body>
          <div class="report-container">
            <div class="page-flex">
              <div>
                <!-- Header with Logo and Title (Exact RBC Format) -->
                <div class="header-layout">
                  <div class="logo-group">
                    <img src="${rabLogoBase64}" alt="RAB Logo" style="height: 46px; object-fit: contain;" />
                    <img src="${coatOfArmsUrl}" alt="Rwanda Coat of Arms" style="height: 42px; object-fit: contain;" />
                  </div>
                  <div class="header-text" style="text-align: right;">
                    <h1>Telemetry Audit — Livestock Division</h1>
                    <p>Vehicle Filter: <strong>${selectedPlate || 'ALL TRACKED GPS VEHICLES (FLEET)'}</strong> &bull; Period: <strong>${timeRange.toUpperCase()}</strong></p>
                  </div>
                </div>

                <div class="divider-line"></div>

                <!-- 4 Metrics Row (Exact RBC Bincard format) -->
                <table class="metrics-table">
                  <tr>
                    <td style="width: 25%;">
                      <div class="metric-label">Total Fleet Logged</div>
                      <div class="metric-value">${targetList.length} Vehicles</div>
                    </td>
                    <td style="width: 25%;">
                      <div class="metric-label">Active In Transit</div>
                      <div class="metric-value">${inTransitCount} Vehicles</div>
                    </td>
                    <td style="width: 25%;">
                      <div class="metric-label">Report Date</div>
                      <div class="metric-value">${new Date().toLocaleDateString('en-GB')}</div>
                    </td>
                    <td style="width: 25%;">
                      <div class="metric-label">Auditor / Officer</div>
                      <div class="metric-value" style="font-size: 12px; font-weight: 700;">${officerName}</div>
                    </td>
                  </tr>
                </table>

                <!-- Sky-Blue RBC Style Table -->
                <table class="rbc-table">
                  <thead>
                    <tr>
                      <th style="width: 10%;">Date</th>
                      <th style="width: 12%;">Plate No</th>
                      <th style="width: 18%;">Driver &amp; Phone</th>
                      <th style="width: 18%;">Permit No &amp; Owner</th>
                      <th style="width: 20%;">Route Corridor</th>
                      <th style="width: 12%;">Cargo Details</th>
                      <th style="width: 5%; text-align: center;">Speed</th>
                      <th style="width: 5%; text-align: center;">Dist</th>
                  ...
                  <tbody>
                    ${targetList.map(v => `
                      <tr>
                        <td>${v.departedTime ? v.departedTime.slice(0, 11) : new Date().toLocaleDateString('en-GB')}</td>
                        <td><strong style="color: #0052cc;">${v.plate}</strong></td>
                        <td>
                          <strong>${v.driverName}</strong><br/>
                          <span style="color: #64748b; font-size: 8px;">${v.driverPhone}</span>
                        </td>
                        <td>
                          <strong>${v.permitNumber}</strong><br/>
                          <span style="color: #64748b; font-size: 8px;">${v.farmerName}</span>
                        </td>
                        <td>${v.route}</td>
                        <td><strong>${v.cargo}</strong></td>
                        <td style="text-align: center;">${v.avgSpeed}</td>
                        <td style="text-align: center;">${v.distance}</td>
              ...
              <div class="footer-section">
                <div style="display: flex; justify-content: space-between; align-items: center; font-weight: 600;">
                  <span>Official RAB Telemetry Report &bull; Generated At: ${new Date().toLocaleString()}</span>
                  <span>Generated By: ${officerName} (RAB)</span>
                </div>
                <div style="margin-top: 3px; text-align: center; font-size: 8px; color: #94a3b8;">
                  Rwanda Agriculture and Animal Resources Development Board (RAB) &bull; National Livestock Tracking System
                </div>
              </div>
