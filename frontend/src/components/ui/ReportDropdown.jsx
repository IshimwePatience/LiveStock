...
import { Search, ChevronDown, Download, Printer, Check } from 'lucide-react';
...
const ReportDropdown = ({ 
  ...
  const [activeCategory, setActiveCategory] = useState('Date Range');
  const [searchQuery, setSearchQuery] = useState('');
  const dropdownRef = useRef(null);

  const categories = ['Date Range', 'Export Format'];
  ...
  const exportOptions = [
    { id: 'CSV', title: 'Export CSV (Excel)', subtitle: 'Download data table in CSV format', icon: Download },
    { id: 'PDF', title: 'Print Official PDF Report', subtitle: 'Generate formatted printable PDF document', icon: Printer },
  ];

  const currentOptions = useMemo(() => {
    const opts = activeCategory === 'Date Range' ? dateOptions : exportOptions;
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        <span>Report</span>
        {timeRange !== 'ALL' && (
          <span className="ml-1 px-1.5 py-0.5 bg-[#0052cc] text-white text-[10px] rounded-full">1</span>
        )}
      </button>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 w-[550px] bg-white rounded-lg shadow-2xl border border-gray-200 z-50 flex flex-col font-sans">
          ...
          <div className="flex items-center justify-between p-3 border-b border-gray-200">
            <h3 className="font-semibold text-gray-800 text-sm">Report Options</h3>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 bg-gray-50/50">
             ...
             <div className="flex items-center gap-2">
               <button
                 ...
                 className="flex items-center gap-1.5 border border-gray-300 hover:bg-gray-100 text-gray-700 text-xs font-semibold px-3 py-1.5 rounded transition"
               >
               ...
               <button
                 ...
                 className="flex items-center gap-1.5 bg-[#0052cc] hover:bg-[#0047b3] text-white text-xs font-semibold px-3 py-1.5 rounded transition"
               >
