...
const ReportDropdown = ({
  ...
  const categories = ['Record Scope', 'Animal Filter', 'Date Range', 'Export Format'];
  ...
  const handleSelectOption = (optId) => {
    if (activeCategory === 'Record Scope') {
      ...
      if (optId === 'CSV') {
        onExportCSV(recordScope, animalFilter);
      } else if (optId === 'PDF') {
        onPrintPDF(recordScope, animalFilter);
      }
      setIsOpen(false);
    }
  };

  const isFiltered = timeRange !== 'ALL' || recordScope !== 'CURRENT_TAB' || animalFilter !== 'ALL';
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {/* Popover Menu */}
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 w-[570px] bg-white rounded-lg shadow-2xl border border-gray-200 z-50 flex flex-col font-sans">
                ...
                {categories.map((cat) => (
                  ...
                  >
                    ...
                    {cat === 'Date Range' && timeRange !== 'ALL' && (
                      <span className="bg-blue-100 text-[#0052cc] text-[9px] px-1.5 py-0.5 rounded-full font-bold">Time</span>
                    )}
                  </button>
                ))}
              </div>
          ...
          <div className="flex items-center justify-between px-4 py-2.5 border-t border-gray-200 bg-gray-50/70">
            <span className="text-xs text-gray-500">
              Active: <strong className="text-gray-800">{scopeOptions.find(s => s.id === recordScope)?.title.split('(')[0].trim() || recordScope}</strong> • <strong className="text-gray-800">{animalOptions.find(a => a.id === animalFilter)?.title.split('(')[0].trim() || 'All Animals'}</strong>
            </span>
